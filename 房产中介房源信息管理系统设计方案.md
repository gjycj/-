...
CREATE TABLE `property_backup` (
  -- 1. 备份表自增主键
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '备份记录ID（自增）',
  
  -- 2. 关联原表的核心字段
  `original_id` bigint(20) NOT NULL COMMENT '关联原楼盘ID（对应property表的id）',
  
  -- 3. 原表所有业务字段（完全复制property表）
  `name` varchar(255) NOT NULL COMMENT '楼盘名称（与原表一致）',
  `address` varchar(512) NOT NULL COMMENT '楼盘地址（与原表一致）',
  `developer` varchar(255) DEFAULT NULL COMMENT '开发商（与原表一致）',
  `property_type` varchar(50) NOT NULL COMMENT '物业类型（住宅/商业/别墅等）',
  `completion_year` int(4) DEFAULT NULL COMMENT '建成年代（与原表一致）',
  `property_company` varchar(255) DEFAULT NULL COMMENT '物业公司（与原表一致）',
  `property_fee` decimal(10,2) DEFAULT NULL COMMENT '物业费（与原表一致）',
  `description` text DEFAULT NULL COMMENT '楼盘描述（与原表一致）',
  `longitude` decimal(10,6) DEFAULT NULL COMMENT '经度（与原表一致）',
  `latitude` decimal(10,6) DEFAULT NULL COMMENT '纬度（与原表一致）',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '原楼盘创建时间（保留历史）',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '原楼盘更新时间（保留历史）',
  `created_by` bigint(20) NOT NULL COMMENT '原楼盘创建人ID（保留历史）',
  `updated_by` bigint(20) NOT NULL COMMENT '原楼盘更新人ID（保留历史）',
  
  -- 4. 备份专用字段
  `backup_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '备份创建时间（原楼盘删除时记录）',
  `delete_reason` varchar(255) DEFAULT NULL COMMENT '原楼盘删除原因',
  `deleted_by` bigint(20) DEFAULT NULL COMMENT '原楼盘删除人ID',
  
  -- 5. 约束与索引（适配备份场景）
  PRIMARY KEY (`id`),
  -- 索引：按原楼盘ID查询备份
  KEY `idx_property_backup_original_id` (`original_id`),
  -- 保留原表的名称和地址索引（支持模糊查询）
  KEY `idx_property_backup_name` (`name`),
  KEY `idx_property_backup_address` (`address`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='楼盘信息备份表（原楼盘删除时自动创建，用于数据恢复）';
...
CREATE TABLE `building_backup` (
  ...
  -- 2. 关联原表的核心字段
  `original_id` bigint(20) NOT NULL COMMENT '关联原楼栋ID（对应building表的id）',
  
  -- 3. 原表业务字段（允许property_id为NULL）
  `property_id` bigint(20) DEFAULT NULL COMMENT '所属楼盘ID（与原楼栋一致，允许为NULL）',
  `name` varchar(100) NOT NULL COMMENT '楼栋名称/编号',
  `total_floor` int(4) DEFAULT NULL COMMENT '总层数',
  `unit_count` int(4) DEFAULT NULL COMMENT '单元数',
  `description` text DEFAULT NULL COMMENT '楼栋描述',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '原楼栋创建时间',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '原楼栋更新时间',
  `created_by` bigint(20) NOT NULL COMMENT '原楼栋创建人ID',
  `updated_by` bigint(20) NOT NULL COMMENT '原楼栋更新人ID',
  ...
  -- 5. 约束与索引
  PRIMARY KEY (`id`),
  KEY `idx_building_backup_original_id` (`original_id`),
  KEY `idx_building_backup_property_id` (`property_id`),
  KEY `idx_building_backup_name` (`name`),
  -- 外键约束（适配SET NULL）
  CONSTRAINT `fk_building_backup_property` 
    FOREIGN KEY (`property_id`) 
    REFERENCES `property` (`id`) 
    ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='楼栋信息备份表';
...
-- 房源信息备份表（存储被物理删除的房源数据）
CREATE TABLE `house_backup` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '备份记录ID（自增主键）',
  `original_id` bigint(20) NOT NULL COMMENT '原房源ID（对应house表的id）',
  -- 原house表的所有字段（完整保留删除时的状态）
  `building_id` bigint(20) NOT NULL COMMENT '所属楼栋ID',
  `house_number` varchar(50) NOT NULL COMMENT '门牌号',
  `layout` varchar(50) NOT NULL COMMENT '户型',
  `area` decimal(10,2) NOT NULL COMMENT '建筑面积(平方米)',
  `inner_area` decimal(10,2) DEFAULT NULL COMMENT '套内面积(平方米)',
  `floor` int(4) NOT NULL COMMENT '所在楼层',
  `total_floor` int(4) DEFAULT NULL COMMENT '总楼层',
  ...
  `description` text DEFAULT NULL COMMENT '房源描述',
  `agent_id` bigint(20) NOT NULL COMMENT '负责经纪人ID',
  `is_valid` tinyint(1) NOT NULL DEFAULT 1 COMMENT '删除时的有效状态：1-有效,0-无效',
  `created_at` datetime NOT NULL COMMENT '原创建时间',
  `updated_at` datetime NOT NULL COMMENT '原最后更新时间',
  `created_by` bigint(20) NOT NULL COMMENT '原创建人ID',
  `updated_by` bigint(20) NOT NULL COMMENT '原最后更新人ID',
  -- 备份相关字段（记录删除操作详情）
  `delete_reason` varchar(512) DEFAULT NULL COMMENT '删除原因（如：错误录入/房源已拆除等）',
  `deleted_at` datetime NOT NULL COMMENT '删除操作时间',
  `deleted_by` bigint(20) NOT NULL COMMENT '执行删除的操作人ID',
  `backup_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '备份记录生成时间（自动记录）',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_house_backup_original_id` (`original_id`) COMMENT '确保同一房源只备份一次',
  KEY `idx_house_backup_deleted_at` (`deleted_at`) COMMENT '按删除时间查询备份',
  KEY `idx_house_backup_building_id` (`building_id`) COMMENT '按楼栋筛选备份数据',
  KEY `idx_house_backup_agent_id` (`agent_id`) COMMENT '按经纪人筛选备份数据'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='房源物理删除备份表（用于数据恢复）';
...
-- 房东备份表
CREATE TABLE `landlord_backup` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '备份记录ID',
  `original_id` bigint(20) NOT NULL COMMENT '原房东ID（关联landlord.id）',
  -- 原房东表所有字段（完整复制）
  `name` varchar(50) NOT NULL COMMENT '姓名',
  `phone` varchar(20) NOT NULL COMMENT '联系电话',
  `id_card` varchar(20) DEFAULT NULL COMMENT '身份证号',
  ...
  `address` varchar(512) DEFAULT NULL COMMENT '联系地址',
  `remark` text DEFAULT NULL COMMENT '备注信息',
  `created_at` datetime NOT NULL COMMENT '原房东创建时间',
  `updated_at` datetime NOT NULL COMMENT '原房东更新时间',
  `created_by` bigint(20) NOT NULL COMMENT '原房东创建人ID',
  `updated_by` bigint(20) NOT NULL COMMENT '原房东更新人ID',
  -- 备份专用字段
  `backup_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '备份创建时间',
  `delete_reason` varchar(255) DEFAULT NULL COMMENT '删除原因',
  `deleted_by` bigint(20) DEFAULT NULL COMMENT '删除人ID',
  -- 索引
  PRIMARY KEY (`id`),
  ...
  KEY `idx_name` (`name`),
  KEY `idx_phone` (`phone`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='房东备份表（删除房东时自动创建）';
...
-- 房源-房东关联备份表（记录被删除的关联关系）
CREATE TABLE `house_landlord_backup` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '备份记录ID',
  `original_id` bigint(20) NOT NULL COMMENT '原关联记录ID（对应house_landlord表的id）',
  `house_id` bigint(20) NOT NULL COMMENT '房源ID',
  `landlord_id` bigint(20) NOT NULL COMMENT '房东ID',
  `is_main` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否主要房东：1-是,0-否',
  `delete_reason` varchar(512) DEFAULT NULL COMMENT '删除原因（如：手动解除关联/房源删除）',
  `deleted_at` datetime NOT NULL COMMENT '删除时间（原记录被删除的时间）',
  `deleted_by` bigint(20) NOT NULL COMMENT '操作人ID（执行删除的用户）',
  `backup_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '备份时间（自动记录）',
  PRIMARY KEY (`id`),
  KEY `idx_house_landlord_backup_original_id` (`original_id`),
  KEY `idx_house_landlord_backup_house_id` (`house_id`),
  KEY `idx_house_landlord_backup_landlord_id` (`landlord_id`),
  KEY `idx_house_landlord_backup_deleted_at` (`deleted_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='房源-房东关联备份表（记录被删除的关联关系）';
...
CREATE TABLE `customer_backup` (
  `id` bigint(20) NOT NULL COMMENT '原客户ID',
  `name` varchar(50) NOT NULL COMMENT '姓名',
  `phone` varchar(20) NOT NULL COMMENT '联系电话',
  `gender` tinyint(1) DEFAULT NULL COMMENT '性别：1-男,2-女',
  `demand` text DEFAULT NULL COMMENT '购房/租房需求描述',
  ...
  `remark` text DEFAULT NULL COMMENT '备注信息',
  `created_at` datetime NOT NULL COMMENT '原创建时间',
  `updated_at` datetime NOT NULL COMMENT '原更新时间',
  `created_by` bigint(20) NOT NULL COMMENT '原创建人ID',
  `updated_by` bigint(20) NOT NULL COMMENT '原更新人ID',
  `backup_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '备份时间',
  `backup_by` bigint(20) NOT NULL COMMENT '执行删除操作的用户ID',
  PRIMARY KEY (`id`),
  KEY `idx_customer_backup_phone` (`phone`),
  KEY `idx_customer_backup_backup_time` (`backup_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='客户信息备份表（存储被删除的客户数据）';
...
CREATE TABLE `agent` (
  ...
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  -- 新增审计字段：创建人ID和更新人ID（与其他业务表保持一致）
  `created_by` bigint(20) NOT NULL COMMENT '创建人ID（操作人）',
  `updated_by` bigint(20) NOT NULL COMMENT '最后更新人ID（操作人）',
  PRIMARY KEY (`id`),
  ...
  -- 可选：添加操作人ID索引，优化按操作人查询的场景
  KEY `idx_agent_created_by` (`created_by`),
  KEY `idx_agent_updated_by` (`updated_by`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='经纪人信息表';

-- 直接添加字段（适用于空表或允许非空字段无默认值的场景）
ALTER TABLE `agent`
  ADD COLUMN `created_by` bigint(20) NOT NULL COMMENT '创建人ID（操作人）' AFTER `updated_at`,
  ADD COLUMN `updated_by` bigint(20) NOT NULL COMMENT '最后更新人ID（操作人）' AFTER `created_by`;

ALTER TABLE `agent_backup` 
ADD COLUMN `created_by` bigint(20) NOT NULL COMMENT '原记录创建人ID（保留历史）',
ADD COLUMN `updated_by` bigint(20) NOT NULL COMMENT '原记录最后更新人ID（保留历史）',
-- 已有的`deleted_by`字段用于记录删除操作人，无需重复添加
ADD KEY `idx_agent_backup_created_by` (`created_by`),
ADD KEY `idx_agent_backup_updated_by` (`updated_by`);
...
CREATE TABLE `agent_backup` (
  ...
  -- 2. 关联原表的核心字段（用于关联原经纪人记录）
  `original_id` bigint(20) NOT NULL COMMENT '原经纪人ID（对应agent表的id）',
  
  -- 3. 原表所有业务字段（完整复制agent表，保留删除时的状态）
  `name` varchar(50) NOT NULL COMMENT '姓名（与原表一致）',
  `employee_id` varchar(20) NOT NULL COMMENT '工号（与原表一致，唯一）',
  `phone` varchar(20) NOT NULL COMMENT '联系电话（与原表一致）',
  `store_id` bigint(20) DEFAULT NULL COMMENT '所属门店ID（与原表一致）',
  `position` varchar(50) DEFAULT NULL COMMENT '职位（与原表一致）',
  `status` tinyint(1) NOT NULL DEFAULT 1 COMMENT '删除时的状态：1-在职,0-离职（保留历史）',
  `created_at` datetime NOT NULL COMMENT '原经纪人创建时间（保留历史）',
  `updated_at` datetime NOT NULL COMMENT '原经纪人最后更新时间（保留历史）',
  
  -- 4. 备份专用字段（记录删除操作详情，用于审计和恢复）
  `backup_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '备份创建时间（经纪人被删除时自动记录）',
  `delete_reason` varchar(512) DEFAULT NULL COMMENT '删除原因（如：员工离职/信息错误等）',
  `deleted_by` bigint(20) NOT NULL COMMENT '执行删除操作的用户ID（记录操作人）',
  
  -- 5. 索引设计（适配查询场景）
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_agent_backup_original_id` (`original_id`) COMMENT '确保同一经纪人只备份一次',
  UNIQUE KEY `uk_agent_backup_employee_id` (`employee_id`) COMMENT '保留工号唯一性，便于按工号查询备份',
  KEY `idx_agent_backup_name` (`name`) COMMENT '按姓名模糊查询备份',
  KEY `idx_agent_backup_phone` (`phone`) COMMENT '按电话查询备份',
  KEY `idx_agent_backup_backup_time` (`backup_time`) COMMENT '按删除时间筛选备份'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='经纪人信息备份表（经纪人被删除时自动创建，用于数据追溯和恢复）';
...
-- 带看记录备份表（存储被删除的带看记录）
CREATE TABLE `visit_record_backup` (
  `id` bigint(20) NOT NULL COMMENT '原带看记录ID',
  `house_id` bigint(20) NOT NULL COMMENT '房源ID（原数据）',
  `customer_id` bigint(20) NOT NULL COMMENT '客户ID（原数据）',
  `agent_id` bigint(20) NOT NULL COMMENT '经纪人ID（原数据）',
  `visit_time` datetime NOT NULL COMMENT '带看时间（原数据）',
  `feedback` text DEFAULT NULL COMMENT '客户反馈（原数据）',
  `next_plan` text DEFAULT NULL COMMENT '下一步计划（原数据）',
  `created_at` datetime NOT NULL COMMENT '原创建时间',
  `updated_at` datetime NOT NULL COMMENT '原更新时间',
  `created_by` bigint(20) NOT NULL COMMENT '原创建人ID',
  `updated_by` bigint(20) NOT NULL COMMENT '原更新人ID',
  `backup_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '备份时间（删除操作时间）',
  `backup_by` bigint(20) NOT NULL COMMENT '执行删除操作的用户ID',
  PRIMARY KEY (`id`),
  KEY `idx_visit_backup_house_id` (`house_id`),
  KEY `idx_visit_backup_customer_id` (`customer_id`),
  KEY `idx_visit_backup_backup_time` (`backup_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='带看记录备份表（存储被删除的带看记录）';
...
-- 房源状态变更备份表
CREATE TABLE `house_status_log_backup` (
  `id` bigint(20) NOT NULL COMMENT '原状态记录ID（与house_status_log表id一致）',
  `house_id` bigint(20) NOT NULL COMMENT '房源ID（原数据）',
  `old_status` tinyint(2) NOT NULL COMMENT '变更前状态',
  `new_status` tinyint(2) NOT NULL COMMENT '变更后状态',
  `reason` varchar(512) DEFAULT NULL COMMENT '变更原因（原数据）',
  `operated_by` bigint(20) NOT NULL COMMENT '原操作人ID',
  `operated_at` datetime NOT NULL COMMENT '原操作时间',
  `delete_reason` varchar(512) DEFAULT NULL COMMENT '删除原因（如：日志归档）',
  `deleted_at` datetime NOT NULL COMMENT '删除时间',
  `deleted_by` bigint(20) NOT NULL COMMENT '执行删除的用户ID',
  `backup_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '备份时间',
  PRIMARY KEY (`id`),
  KEY `idx_house_status_log_backup_house_id` (`house_id`),
  KEY `idx_house_status_log_backup_operated_at` (`operated_at`),
  KEY `idx_house_status_log_backup_deleted_at` (`deleted_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='房源状态变更记录备份表（存储被删除的状态日志）';
```
